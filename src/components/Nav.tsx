    ...
    return (
        <nav className="w-full decoration-clone bg-gradient-to-b from-green-500 to-blue-600 h-14 flex justify-around items-center">
            ...
            <div className="rounded-t-[10px] md:rounded-lg flex items-stretch justify-end px-2 py-1 md:m-1">
                    ...
                    <div className="rounded-t-[10px] md:rounded-lg flex items-stretch justify-end px-2 py-1 md:m-1">
                        <Link onClick={async () => await axios.post("/logout") } to="/login"  className="bg-black bg-opacity-20 hover:bg-opacity-30 flex mr-1 px-3 py-2 text-xs font-medium text-white rounded-md focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-white focus-visible:ring-opacity-50">Logout</Link>
